...
class ChatConsumer(WebsocketConsumer):
    def send_error_code(self, code):
        self.send(
            text_data=json.dumps(
                {"message": json.dumps({"kind": "kaboom", "code": code})}
            )
        )
    ...
    def create_room(self):
        ...
        self.send(
            text_data=json.dumps(
                {"message": json.dumps({"kind": "move-to", "key": key})}
            )
        )
    ...
    def send_update(self, task_list):
        ...
        async_to_sync(self.channel_layer.group_send)(
            ...
            {
                "type": "chat_message",
                "message": json.dumps({"kind": "todo_init", "todos": tasks_dict}),
            },
        )
    ...
    def chat_message(self, event):
        ...
        self.send(text_data=json.dumps({"sent_by": "chat_message", "message": message}))
